...
class simple_html_dom_node {
    ...
    /**
     * Define um atributo para a tag
     * @param String $str
     */
    function atr($atributo, $str) {
    	$this->$atributo = $str;
    }
    
    /**
     * Insere uma string entre as tags
     * @param String $str
     * @param Boolean $concat: Indica se deve se o conteúdo será substituido ou concatenado
     */
    function setIn($conteudo, $reset = false) {
    	if(is_a($conteudo, 'DTag')) {
    		$conteudo = $conteudo->toStr();
    	}
    	if(!$reset) {
    		$this->innertext .= "\n".$conteudo."\n";
    	} else {
...
class simple_html_dom {
    ...
    public $linkHtml = null; // Vari�vel n�o original. Inserida para corrigir bug.
    public $aux = null; // Vari�vel n�o original. Inserida para corrigir bug.
    public $root = null;
    public $nodes = array();
    ...
    function load_file() {
        $args = func_get_args();
        $this->linkHtml = $args[0];
        $this->load(call_user_func_array('file_get_contents', $args), true);
        if (!is_object($this->root)) {
            $this->load_file($args[0]);
        }
    ...
    // Função que troca o caminho relativo de uma imagem pelo caminho absoluto
    function show($cam = true, $filepath = '') {
        $str = $this->save();
        if ($str) {
            return $str;
        } else {
